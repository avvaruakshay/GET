...
const axis = function(Obj){
  ...
  // Orientation of axis
  if (orient === 'bottom'){ axis = d3.axisBottom(scale);}
  else if (orient === 'top') { axis = d3.axisTop(scale);}
  else if (orient === 'left') { axis = d3.axisLeft(scale);}
  else if (orient === 'right') { axis = d3.axisRight(scale);}

  // Customising for number of ticks or the Tick values
  if (Obj.hasOwnProperty('ticks')) {
    ticks = Obj.ticks;
    if (typeof ticks === 'number'){ axis = axis.ticks(ticks);}
    else if (ticks instanceof Array) { axis = axis.tickValues(ticks);}
  }

  // Specifying the tickFormat
  if(Obj.hasOwnProperty('tickformat')){
    tickFormat = Obj.tickFormat;
